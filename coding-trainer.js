...
function ctUpdatePreview(type, code) {
  ...
  if (!iframe || !consoleOut || !label) return;

  // Always reset both panels first to avoid stale display
  ...
  if (lang === 'html') {
    ...
    // Force iframe reset: blank first, then set new content after a tick
    iframe.srcdoc = '';
    requestAnimationFrame(() => {
      try { iframe.srcdoc = code; } catch(e) { console.warn('iframe srcdoc error:', e); }
    });
  } else if (lang === 'css') {
    iframe.style.display = 'block';
    ...
    iframe.srcdoc = '';
    requestAnimationFrame(() => {
      try { iframe.srcdoc = demoHTML; } catch(e) { console.warn('iframe srcdoc error:', e); }
    });
  } else if (lang === 'js') {
...
function ctRunJSPreview(code) {
  ...
  try {
    ...
    // Stub out browser APIs with silent no-ops so code runs without crashing
    const stubDocument = new Proxy({}, {
      get: () => new Proxy(() => null, { get: () => () => null, apply: () => null }),
    });
    const stubWindow = new Proxy({}, {
      get: (_, k) => k === 'console' ? sandboxConsole : new Proxy(() => null, { get: () => () => null }),
    });
    const stubLS = new Proxy({}, { get: () => () => null, set: () => true });

    const fn = new Function('console', 'document', 'window', 'localStorage', '"use strict";\n' + code);
    fn(sandboxConsole, stubDocument, stubWindow, stubLS);
  } catch(e) {
